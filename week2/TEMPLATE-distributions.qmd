...
#..........................import data...........................
mko <- read_csv(here::here("MEDS", "eds-240", "EDS-240-class-examples", "week2", "data", "mohawk_mooring_mko_20250117.csv"))
...
mko_clean <- mko |>
  ...
  # replace 9999s with NAs ---- # why did they do this?
...
mko_clean %>% 
  ...
  geom_histogram(position = "identity", alpha = .4) + theme_bw() 
...
mko_clean %>% 
  ...
  geom_density( alpha = .4, fill = "darkgrey") + theme_bw() + facet_wrap(~month_name)
...
mko_clean |> 
  ...
  filter(month_name %in% c("April", "June", "October")) %>%
  ggplot(aes(x = Temp_bot, fill = month_name)) +
  geom_histogram(position = "identity", alpha = 0.7, col = "darkgrey", binwidth = 1) + 
  scale_fill_manual(values = c("#2C5374", "#ADD8E6", "#8B3A3A")) + 
  theme_bw()
...
mko_clean |> 
  filter(month_name %in% c("April", "June", "October")) %>%
  ggplot(aes(x = Temp_bot, fill = month_name)) +
  geom_density(alpha = 0.5, adjust = .2) + theme_bw()
```
...
R will order the levels alphabetically.
:::
...
What should you carefully consider when checking the smoothing assumptions of your density curve against a histogram?
:::
...
## good way to check if density is reflective of hist dist
```
...
mko_clean %>% 
  ...
  geom_density_ridges(rel_min_height = 0.001, scale = 3) + theme_ridges()
...
Benefits: Only way maps to ggplot... not changing original data...quick way of reorganizing.
:::
...
ggplot(penguins, aes(x = species, y = body_mass_g)) +
  ...
  geom_beeswarm(size = 1) # do not need boxplot; can be stand alone
```
...
ggplot(penguins, aes(x = species, y = body_mass_g, col = as.factor(year))) +
  geom_boxplot(outlier.shape = NA) + 
  theme_bw() + 
  geom_point(alpha = .5, position = position_jitterdodge(jitter.width = .2))
```
...
#........................violin + box plot.......................
ggplot(mko_clean, aes(x = month_name, y = Temp_bot)) +
  geom_violin() + geom_boxplot(width = .1, fill = "grey30", alpha = .5) + 
  scale_x_discrete(limits = rev(month.name)) + 
  coord_flip()
```
